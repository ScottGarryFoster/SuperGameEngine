...
FPoint RectangleInt::CloestPointTo(const FPoint& other) const
{
    ...
    return FPoint(closestX, closestY);
}
...
{
    // We tried caching this but it appeared not to work in all cases.
    // This should be investigated.
    return GenerateCenter(m_location, m_size);
}

FVector2D SuperGameEngine::RectangleInt::GenerateCenter(const FPoint& location, const FPoint& size) const
{
...
FVector2D SuperGameEngine::RectangleInt::GenerateCenter(const FVector2D& location, const FVector2D& size) const
{
    FVector2D newCenter = FVector2D(
        location.GetX() + size.GetX() / 2.0f,
        location.GetY() + size.GetY() / 2.0f);
