...
namespace StandardCLibrary
{
    ...
    static FileSystem::copy_options Convert(CopyFileOptions options)
    {
        ...
        if (ECopyOptions::HasOption(options, CopyFileOptions::UpdateExistingFile))
        {
            returnOptions |= FileSystem::copy_options::update_existing;
            haveAtLeastOneOption = true;
        }

        if (ECopyOptions::HasOption(options, CopyFileOptions::RecursiveDirectories))
        {
            returnOptions |= FileSystem::copy_options::recursive;
            haveAtLeastOneOption = true;
        }

        if (ECopyOptions::HasOption(options, CopyFileOptions::CopySymlinks))
        {
            returnOptions |= FileSystem::copy_options::copy_symlinks;
            haveAtLeastOneOption = true;
        }

        if (ECopyOptions::HasOption(options, CopyFileOptions::SkipSymlinks))
        {
            returnOptions |= FileSystem::copy_options::skip_symlinks;
            haveAtLeastOneOption = true;
        }

        if (ECopyOptions::HasOption(options, CopyFileOptions::DirectoriesOnly))
        {
            returnOptions |= FileSystem::copy_options::directories_only;
            haveAtLeastOneOption = true;
        }
        if (ECopyOptions::HasOption(options, CopyFileOptions::CreateSymlinks))
        {
            returnOptions |= FileSystem::copy_options::create_symlinks;
            haveAtLeastOneOption = true;
        }
        if (ECopyOptions::HasOption(options, CopyFileOptions::CreateHardLinks))
        {
